...
func load_settings() -> void:
	...
	if settings_data != null:
		...
		set_resolution(settings_data.resoultion, settings_data.resolution_index)
...
func set_resolution(resolution : Vector2i, resolution_index : int) -> void:
	get_tree().root.content_scale_size = resolution
	
	settings_data.resoultion = resolution
